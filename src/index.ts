import MiddlewareFunction from './middlewareFn';
import compose from './compose'

let middlewareManagerHash: any[] = [];
...
export class MiddlewareManager {
    ...
    static subscriptions: MiddlewareFunction[] = [];
    static useInMiddleware(...middlewares: MiddlewareFunction[]): any {
        for (let middleware of middlewares) {
            MiddlewareManager.subscriptions.push(middleware);
        }
    }
     ...
     * @param {...any} middlewareObjects Middleware objects.
     * @return {any} this
     */
    constructor(target: any, ...middlewareObjects: object[]) {
        let instance = middlewareManagerHash.find((x: any) => x._target === target);

        // A target can only has one instance
     ...
     * @return {MiddlewareManager} this
     */
    public use(methodName: string | object, ...middlewares: any[]): object {
            ...
            Array.prototype.slice.call(arguments).forEach(arg => {
                ...
                typeof arg === 'object' &&
                    (arg.middlewareMethods ||
                        ...
                        typeof arg[key] === 'function' && this.isMethodValid(key.toString()) && this.applyToMethod(key.toString(), arg[key].bind(arg));
                    });
            });
        } else {
     ...
     * @param behaviorName 
     */
    public useNamed(methodName: string | object, behaviorName: string): MiddlewareManager {
        this.applyToMethod(methodName, ...MiddlewareManager.subscriptions.filter(x => x.name == behaviorName).map(x => x.handler));
        return this;
    }
     ...
     * @param methodName 
     */
    private isMethodValid = (methodName: string): boolean => !/^_+|_+$|constructor/g.test(methodName);
     ...
     * @returns {void}
     */
    private applyToMethod(methodName: string | object, ...middlewares: Function[]): void {
        if (typeof methodName === 'string' && this.isMethodValid(methodName)) {
